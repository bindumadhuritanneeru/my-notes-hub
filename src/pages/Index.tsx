...
const Index = () => {
  ...
  useEffect(() => {
    getNotes().then((notes) => setCount(notes.length));
  }, []);
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          <Button asChild size="lg">
            <Link to="/create"><Plus className="mr-2 h-5 w-5" />Create Note</Link>
          </Button>
          <Button asChild variant="outline" size="lg">
            <Link to="/notes"><Eye className="mr-2 h-5 w-5" />View Notes</Link>
          </Button>
        </div>
      </motion.div>
    </div>
